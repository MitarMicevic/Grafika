...
//        modelGrass = glm::scale(modelGrass, glm::vec3(programState->backpackScale));    // it's a bit too big for our scene, so scale it down
        ...
        modelGrass = glm::scale(modelGrass, glm::vec3(0.07f, 0.01f, 0.06f));
...
void processInput(GLFWwindow *window) {
    ...
    if (glfwGetKey(window, GLFW_KEY_C) == GLFW_PRESS && !blinnKeyPressed)
    {
        blinn = !blinn;
...
void DrawImGui(ProgramState *programState) {
...

    {
        ...
        ImGui::DragFloat("Backpack scale", &programState->grassScale, 0.05, 0.1, 4.0);
